...
def get_model_with_efnb3():
    backbone_model = efn.EfficientNetB3(include_top=False, input_tensor=None, input_shape=IM_SIZE, weights='imagenet')
    # backbone_model.summary()
    model = Sequential()
    model.add(backbone_model)
    # (None, 1536) -> 16 * 16 * 9
    ...
    # (None, 75264)   -> 224, 336
    #  (None, 7, 7, 1792) -> 224, 392
    model.add(tf.keras.layers.Reshape((224, 336)))
    # model.add(tf.keras.layers.Reshape((224, 392)))
    # 注释这部分单向lstm代码
    model.add(LSTM(units=256, return_sequences=True))
    model.add(Dropout(0.5))
...
# reload callback
# define callback
# check_point_path = 'models/new_efnb3_more_lstm_15_5s/EfficientNetB3.ckpt'
...
test_generator = test_datagen.flow_from_directory(directory='test_split_img_new_15_5s', target_size=IM_SIZE[:2],
...
# df.to_csv('submit_lstm_maxvote.csv', index=False, header=False)

df = pd.Series(answer_list_prob)
df.to_csv('submit_lstm_maxprobxxx.csv', index=False, header=False)
